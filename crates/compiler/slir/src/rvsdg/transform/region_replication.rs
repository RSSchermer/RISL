    ...
    fn replicate_node(&mut self, node: Node) -> Node {
        ...
        match self.rvsdg[node].kind() {
            ...
            Simple(OpRoundToEven(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpSaturate(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpFloor(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpCeil(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpClamp(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpFract(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpFusedMulAdd(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpTrunc(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpSqrt(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            ...
            Simple(OpLog2(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpPowf(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpStep(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpSmoothStep(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            ...
            Simple(OpAtanh(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpToRadians(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
            Simple(OpToDegrees(op)) => self
                .prepare_replicate_intrinsic_node(op)
                .apply(&mut self.rvsdg),
