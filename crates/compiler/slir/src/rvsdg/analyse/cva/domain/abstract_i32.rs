...
/// Returns the number that cancels multiplication by an odd `value` when arithmetic wraps at 32
/// bits.
///
/// In other words, multiplying the returned number by `value` produces `1`.
fn wrapping_inverse(value: u32) -> u32 {
    debug_assert!(value % 2 == 1);

    let mut inverse = value;

    // Each pass doubles the number of low bits for which `inverse * value` is equal to `1`.
    // Five passes are enough to make all 32 bits correct.
    for _ in 0..5 {
...
impl AbstractI32 {
    ...
    ///
    /// The constraints returned include the prior constraints on the operands, not just the
    /// additional constraints implied by the result. Refinement is exact for multipliers `0`, `1`,
    /// and `-1`. When the multiplier and result are both single values, this also finds the values
    /// that produce that exact result when multiplication wraps at 32 bits. Otherwise, an operand
    /// interval is retained if its abstract product with the other operand may intersect the result.
    ///
    /// Returns `("bottom", "bottom")` if the abstract inverse evaluation produced a contradiction.
    pub fn abstract_mul_inv(&self, other: &Self, res: &Self) -> (Self, Self) {
        let refine_operand =
            |value: &Self, multiplier: &Self| match (multiplier.to_singleton(), res.to_singleton())
            {
                // We special-case the simple variations where one operand is `0`, `1`, or `-1`
                // singleton value.
                (Some(0), _) if res.contains(0) => value.clone(),
                (Some(0), _) => Self::bottom(),
                (Some(1), _) => value.refine(res),
                (Some(-1), _) => value.refine(&res.abstract_neg()),

                // If one operand and the result are both singletons, we attempt to infer tighter
                // bounds on the other operand. However, this is complicated by the fact that we are
                // dealing with wrapping multiplication. For example, given `"top" * 2 == 0`, it
                // may at first glance seem we can improve the LHS constraints from "top" all the
                // way down to `0`. However, under wrapping multiplication `i32::MIN * 2` also
                // produces `0`. The implementation currently does not split the prior constraining
                // intervals into multiple intervals (this example would only need two, but in the
                // general case we may need many). Therefore, the LHS constraint we end up
                // inferring is `I32::MIN..=0`. While this is clearly very conservative w.r.t. the
                // tightest possible constraints (`{i32::MIN, 0}`), it is still a large improvement
                // over "top" (a.k.a. `I32::MIN..=I32::MAX`).
                (Some(multiplier), Some(result)) => {
                    // Reinterpret the signed values as their 32-bit patterns. Wrapping `i32`
                    // multiplication is the same as multiplying these patterns and discarding
                    // everything above the lowest 32 bits.
                    let result = result as u32;
                    let multiplier = multiplier as u32;

                    // A power of two is the only kind of factor that `multiplier` can share with
                    // `2^32`. Every possible product contains this factor, so the requested result
                    // must contain it too. If it does not, there is no value that can produce the
                    // result.
                    let common_factor = 1u64 << multiplier.trailing_zeros();

                    if result as u64 % common_factor != 0 {
                        Self::bottom()
                    } else {
                        // Remove the shared power of two from the multiplier, result, and wrapping
                        // range. The reduced multiplier is odd, which means it has an inverse in
                        // the reduced wrapping range. Multiplying by that inverse gives one
                        // solution (`base`).
                        let modulus = I32_MODULUS / common_factor as i64;
                        let reduced_multiplier = (multiplier as u64 / common_factor) as u32;
                        let reduced_result = result as u64 / common_factor;
                        let base = (reduced_result * wrapping_inverse(reduced_multiplier) as u64
                            % modulus as u64) as i64;

                        // All other solutions are exactly `modulus` apart. For each existing
                        // interval, find its first and last solutions. We keep the interval between
                        // those endpoints because this abstract value can store intervals, but not
                        // an arbitrarily long list of separated individual solutions.
                        Self::from_intervals(value.0.iter().filter_map(|interval| {
                            let start = *interval.start() as i64;
                            let end = *interval.end() as i64;
                            let first = start + (base - start).rem_euclid(modulus);

                            (first <= end).then(|| {
                                let last = end - (end - base).rem_euclid(modulus);
                ...
                // In all other cases, we simply conservatively retain intervals if the result of
                // multiplication with the other operand intersects the result at all.
                _ => Self::from_intervals(value.0.iter().filter_map(|interval| {
                    let interval_value = Self::from_intervals([interval.clone()]);

                    (!interval_value
                        .abstract_mul(multiplier)
                        .refine(res)
                        .is_bottom())
                    .then(|| interval.clone())
                })),
        ...
        let refinements = (refine_operand(self, other), refine_operand(other, self));
...
mod tests {
    ...
    fn abstract_mul_inv() {
        assert_eq!(
            AbstractI32::from_intervals([2..=3, 10..=12]).abstract_mul_inv(
                &AbstractI32::from_intervals([4..=5, 20..=21]),
                &AbstractI32::from_intervals([8..=15]),
            ),
            (
                AbstractI32::from_intervals([2..=3]),
        ...
        assert_eq!(
            AbstractI32::top().abstract_mul_inv(
                &AbstractI32::from_constant(3),
                &AbstractI32::from_constant(1),
            ),
            (
                AbstractI32::from_constant(-1431655765),
                AbstractI32::from_constant(3),
            )
        );
        assert_eq!(
        ...
        assert_eq!(
            AbstractI32::from_intervals([-10..=10]).abstract_mul_inv(
                &AbstractI32::from_constant(4),
                &AbstractI32::from_constant(0),
            ),
            (AbstractI32::from_constant(0), AbstractI32::from_constant(4))
        );
        // Both `0` and `i32::MIN` produce `0` when multiplied by `2`, because the latter product
        // wraps at 32 bits. The current implementation keeps the hull interval around these two
        // possible values as a conservative approximation, hence the wide `i32::MIN..=0` constraint
        // inferred for the LHS. We could in the future possibly split such intervals it wouldn't
        // exceed MAX_INTEGER_INTERVALS.
        assert_eq!(
            AbstractI32::top().abstract_mul_inv(
                &AbstractI32::from_constant(2),
                &AbstractI32::from_constant(0),
            ...
            (
                AbstractI32::from_intervals([i32::MIN..=0]),
                AbstractI32::from_constant(2),
            )
        );
        assert_eq!(
            AbstractI32::from_intervals([-10..=10]).abstract_mul_inv(
                &AbstractI32::from_constant(6),
                &AbstractI32::from_constant(18),
            ),
            (AbstractI32::from_constant(3), AbstractI32::from_constant(6))
        );
        assert_eq!(
            AbstractI32::top().abstract_mul_inv(
                &AbstractI32::from_constant(2),
        ...
        assert_eq!(
            AbstractI32::from_intervals([1..=10]).abstract_mul_inv(
                &AbstractI32::from_constant(1),
                &AbstractI32::from_intervals([4..=6]),
            ),
            (
        ...
        assert_eq!(
            AbstractI32::from_constant(-1).abstract_mul_inv(
                &AbstractI32::from_intervals([1..=10]),
                &AbstractI32::from_intervals([-6..=-4]),
            ),
            (
                AbstractI32::from_constant(-1),
                AbstractI32::from_intervals([4..=6]),
        ...
        assert_eq!(
            AbstractI32::from_intervals([1..=3]).abstract_mul_inv(
                &AbstractI32::from_constant(0),
                &AbstractI32::from_constant(0),
            ),
            (
        ...
        assert_eq!(
            AbstractI32::from_constant(i32::MAX).abstract_mul_inv(
                &AbstractI32::from_constant(2),
                &AbstractI32::from_constant(-2),
            ),
            (
        ...
        assert_eq!(
            AbstractI32::from_intervals([1..=3])
                .abstract_mul_inv(&AbstractI32::from_intervals([4..=6]), &AbstractI32::top()),
            (
                AbstractI32::from_intervals([1..=3]),
                AbstractI32::from_intervals([4..=6]),
            )
        ...
        assert_eq!(
            AbstractI32::top().abstract_mul_inv(&AbstractI32::top(), &AbstractI32::bottom()),
            (AbstractI32::bottom(), AbstractI32::bottom())
        );
    }
