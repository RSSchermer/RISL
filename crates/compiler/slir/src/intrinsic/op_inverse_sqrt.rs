...
pub struct OpInverseSqrt;

impl Intrinsic for OpInverseSqrt {
    fn process_args(
        &self,
        ty_registry: &TypeRegistry,
        args: impl IntoIterator<Item = Type>,
    ) -> Result<Option<Type>, String> {
        let arg = expect_one_arg!("inverse_sqrt", args);

        match &*ty_registry.kind(arg) {
            TypeKind::Scalar(ScalarKind::F32) => Ok(Some(arg)),
            _ => Err(format!(
                "intrinsic `inverse_sqrt` expects a floating-point scalar argument, found `{}`",
