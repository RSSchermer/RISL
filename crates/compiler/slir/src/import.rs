...
impl FunctionImporter {
    ...
    fn import_statement(
        ...
        match &src_cfg[src_stmt] {
            ...
            StatementData::OpSaturate(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpClamp(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpFract(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpFusedMulAdd(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpTrunc(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpSqrt(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            ...
            StatementData::OpPowf(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpStep(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpSmoothStep(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            ...
            StatementData::OpToRadians(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
            StatementData::OpToDegrees(op) => {
                self.import_stmt_intrinsic_op(src_mod, (dst_mod, dst_cfg), dst_bb, op)
            }
