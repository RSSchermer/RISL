#![cfg(feature = "test_slice_split_first")]
...
test_runner! {
    ...
    shader: {
        ...
        let result = if let Some((first, rest)) = slice.split_first() {
            let mut sum = 0;
            for v in rest {
                sum += *v;
            }
...
async fn run() -> Result<(), Box<dyn Error>> {
    ...
    // Case 1: Sub-slice [10, 20, 30, 40] -> first: 10, rest: [20, 30, 40] (sum: 90)
    assert_eq!(
    ...
    // Case 2: Sub-slice [20, 30, 40] -> first: 20, rest: [30, 40] (sum: 70)
    assert_eq!(
    ...
    // Case 3: Sub-slice [40] -> first: 40, rest: [] (sum: 0)
    assert_eq!(
    ...
    // Case 4: Sub-slice [] -> first: 99, rest_sum: 99
    assert_eq!(
