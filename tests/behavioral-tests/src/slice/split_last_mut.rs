#![cfg(feature = "test_slice_split_last_mut")]
...
test_runner! {
    ...
    shader: {
        ...
        let result = if let Some((last, rest)) = slice.split_last_mut() {
            *last += 100;
...
async fn run() -> Result<(), Box<dyn Error>> {
    ...
    // Case 3: Sub-slice [10, 20, 30]
    // mutate: last (30) -> 130, rest ([10, 20]) -> [11, 21] (sum: 32)
    assert_eq!(
    ...
    // Case 4: Sub-slice [10, 20, 30, 40]
    // mutate: last (40) -> 140, rest ([10, 20, 30]) -> [11, 21, 31] (sum: 63)
    assert_eq!(
